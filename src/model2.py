...
class ExactLFM(gpx.base.Module):
    ...
    def mean_function(self, x: Num[Array, "N D"]) -> Float[Array, "N O"]:
        ...
        print(f" input to mean shape {x.shape}")
    ...
    # Define mean function
    def mean_function2(self, x: Num[Array, "N D"]) -> Float[Array, "N O"]:
        f = jnp.array(x[:, 2:], dtype=int)
        ...
        mean = jnp.vstack((jnp.array([0]), mean))
    ...
    def latent_predict(
        ...
        # First four genes only
        x = x[:28]
        y = y[:28]
        variances = variances[:28]
    ...
    def latent_predict2(
        ...
        # Massage f(0) = 0
        x = jnp.vstack((jnp.array([0, 0, 1]), x))
        y = jnp.vstack((jnp.array([0]), y))
        variances = jnp.vstack((jnp.array([0]), variances))
